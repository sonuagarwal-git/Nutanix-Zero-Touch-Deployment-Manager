...
function Get-PrismCentralCluster {
    ...
    if (-not $pcEntry) { Write-LogMessage "Could not find PC entry in clusters list" -Level Error; return $null }

    $pcVMName   = $pcEntry.spec.name
    $searchBody = @{ kind = "vm"; filter = "vm_name==$pcVMName"; length = 10 }
    try {
        $vmSearch = Invoke-PrismAPI -Method POST -Endpoint "vms/list" -Body $searchBody
        if ($vmSearch.entities.Count -eq 0) {
            Write-LogMessage "VM search for '$pcVMName' returned no results" -Level Error; return $null
        }
        $pcVMDetail      = Invoke-PrismAPI -Method GET -Endpoint "vms/$($vmSearch.entities[0].metadata.uuid)"
        $hostClusterUUID = $null
        if ($pcVMDetail.spec.cluster_reference)       { $hostClusterUUID = $pcVMDetail.spec.cluster_reference.uuid }
        elseif ($pcVMDetail.status.cluster_reference) { $hostClusterUUID = $pcVMDetail.status.cluster_reference.uuid }

        if (-not $hostClusterUUID) { Write-LogMessage "Could not extract cluster reference from PC VM" -Level Error; return $null }
        ...
        if ($hostCluster) {
            Write-LogMessage "Prism Central runs on cluster: $($hostCluster.spec.name)" -Level Success
            return $hostCluster
        }
        Write-LogMessage "Could not match UUID $hostClusterUUID to a real cluster" -Level Error; return $null
    }
    catch { Write-LogMessage "Error locating PC cluster: $($_.Exception.Message)" -Level Error; return $null }
}
