...
.PARAMETER Credential
    PSCredential for a DHCP admin or read-only account (e.g. VESTAS\SVC-DKCDC-NTX-AUTO).
...
.EXAMPLE
    ...
    .\Query-DhcpLeases.ps1 -DhcpServer 10.0.10.80 -ScopeId 10.0.113.0,10.0.114.0 `
        -Username "VESTAS\SVC-DKCDC-NTX-AUTO"

.EXAMPLE
    # Query ALL scopes using service account credentials
    .\Query-DhcpLeases.ps1 -DhcpServer 10.0.10.80 -All -Username "VESTAS\SVC-DKCDC-NTX-AUTO"
#>
...
foreach ($scope in $ScopeId) {
    ...
    try {
        ...
        elseif ($msg -match 'Failed to get version|access is denied|access denied|unauthorized|privilege' ) {
            ...
            Write-Host "    .\Query-DhcpLeases.ps1 -DhcpServer $DhcpServer -ScopeId $scope -Username 'VESTAS\SVC-DKCDC-NTX-AUTO'" -ForegroundColor DarkYellow
        }
        else {
