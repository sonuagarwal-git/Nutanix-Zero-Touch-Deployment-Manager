...
- **18-step automated pipeline** — Phoenix boot through NCC health check, fully orchestrated
...
When a deployment starts, `Start-Pipeline.ps1` runs up to 18 sequential steps:
...
> Step 18 (NCC Health Check) is optional — runs only when `health_check.enabled: true` is set in the config.
